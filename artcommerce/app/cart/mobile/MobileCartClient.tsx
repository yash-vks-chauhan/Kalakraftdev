      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-md mx-auto text-center">
          ...
          <div className="bg-white p-8 rounded-xl border-2 border-gray-200">
            <Link href="/products" className="block w-full text-center rounded-full border-2 border-black px-6 py-3 font-semibold text-black">Browse Products</Link>
          </div>
        </div>
      </main>
    )
      ...
      <main className="container mx-auto px-4 pt-4 pb-28" data-testid="mobile-cart-page">
        ...
        <ul className="bg-white rounded-2xl border border-gray-300 divide-y divide-gray-200">
          {cartItems.map((item) => {
            ...
            const maxQty = hasStockInfo && (item.product.stockQuantity as number) > 0 ? (item.product.stockQuantity as number) : undefined
              ...
              <li key={item.id} className="flex items-center gap-3 p-3">
                <Link href={`/products/${item.product.id}`} className="shrink-0">
                  {Array.isArray(item.product.imageUrls) && item.product.imageUrls[0] ? (
                    <img src={item.product.imageUrls[0]} alt={item.product.name || 'Product image'} width={80} height={80} className="h-20 w-20 object-cover rounded-xl border border-black" loading="lazy" onError={(e) => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} />
                  ) : (
                    <div className="h-20 w-20 rounded-xl border border-black bg-gray-50 flex items-center justify-center text-gray-400">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-6 w-6"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    </div>
                  )}
                </Link>
                ...
                <div className="min-w-0 flex-1">
                  <Link href={`/products/${item.product.id}`} className="block">
                    <h2 className="text-base font-semibold text-gray-900 leading-tight line-clamp-2">{item.product.name}</h2>
                  </Link>
                  <div className="mt-2 flex items-center justify-between">
                    <div className="text-sm font-medium text-gray-900">₹{item.product.price.toFixed(2)} <span className="text-gray-400">×</span> {item.quantity}<span className="ml-2 text-gray-500">=</span> <span className="ml-1">₹{(item.product.price * item.quantity).toFixed(2)}</span></div>
                    {hasStockInfo && (<span className={`text-xs ${isOut ? 'text-red-600' : 'text-gray-500'}`}>{isOut ? 'Out of stock' : 'In stock'}</span>)}
                  </div>
                  <div className="mt-2 flex items-center justify-between">
                    {isOut ? (
                      <span className="inline-flex items-center rounded-full border-2 border-red-600 text-red-600 px-3 py-2 text-xs font-semibold select-none">Out of stock</span>
                    ) : (
                      <div className="relative inline-flex items-center rounded-full border-2 border-black overflow-hidden shadow-sm">
                        <button type="button" aria-label="Decrease quantity" onClick={handleDecrease} disabled={isOut || item.quantity <= 1 || isBusy} className="px-5 py-2 text-black disabled:opacity-40 transition-all active:translate-y-0.5 active:scale-95 focus:outline-none hover:bg-gray-100 ring-offset-1 active:ring-2 active:ring-black">−</button>
                        <input id={`qty-${item.id}`} type="number" inputMode="numeric" min={1} max={maxQty} value={item.quantity} onChange={async (e) => { const nextVal = Math.max(1, parseInt(e.target.value, 10) || 1); setUpdating((prev) => ({ ...prev, [item.id]: true })); try { const ok = await updateCartItem(item.id, nextVal); if (!ok) router.refresh(); } finally { setUpdating((prev) => ({ ...prev, [item.id]: false })); } }} className="w-16 text-center text-black bg-white border-l-2 border-r-2 border-black focus:outline-none" disabled={isOut || isBusy} />
                        <button type="button" aria-label="Increase quantity" onClick={handleIncrease} disabled={isOut || (maxQty !== undefined && item.quantity >= maxQty) || isBusy} className="px-5 py-2 text-white bg-black disabled:opacity-40 transition-all active:translate-y-0.5 active:scale-95 focus:outline-none hover:bg-black/90 ring-offset-1 active:ring-2 active:ring-black">+</button>
                        {isUpdating && (<div className="absolute -right-8 top-1/2 -translate-y-1/2"><span className="h-4 w-4 rounded-full border-2 border-black border-t-transparent inline-block animate-spin" /></div>)}
                      </div>
                    )}
                    {showQtyError && (<div className="ml-3 text-xs text-red-600">Max {maxQty as number} available</div>)}
                    <button onClick={async () => { setRemovingId(item.id); try { await removeFromCart(item.id) } finally { setRemovingId(null) } }} className="ml-4 inline-flex items-center gap-2 text-sm rounded-full border-2 border-black px-4 py-2 font-semibold text-black hover:bg-black hover:text-white active:translate-y-px transition-all disabled:opacity-50" disabled={isBusy} aria-label="Remove item">
                      {removingId === item.id ? (<span className="h-4 w-4 rounded-full border-2 border-black border-t-transparent inline-block animate-spin" />) : (
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-5 w-5"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 7h6m-7 0h8a1 1 0 001-1V5a1 1 0 00-1-1h-3.5l-1-1h-3l-1 1H6a1 1 0 00-1 1v1z" /></svg>
                      )}
        ...
        <div className="fixed left-0 right-0 bg-white z-[1100]" style={{ bottom: '64px' }}>
          <div className="container mx-auto px-4 py-2 border-t-2 border-gray-200">
            <button onClick={() => setIsSummaryOpen((v) => !v)} className="w-full flex items-center justify-between text-sm font-medium"><span>Order summary</span><span>{isSummaryOpen ? '−' : '+'}</span></button>
            {isSummaryOpen && (
              <div className="mt-2 space-y-1 text-sm">
                <div className="flex justify-between"><span className="text-gray-600">Subtotal</span><span>₹{subtotal.toFixed(2)}</span></div>
                {discountAmount > 0 && (<div className="flex justify-between text-green-700"><span>Discount</span><span>−₹{discountAmount.toFixed(2)}</span></div>)}
                <div className="flex justify-between"><span className="text-gray-600">Shipping (est)</span><span>₹0.00</span></div>
                ...
                <button onClick={() => setPromoOpen(true)} className="mt-2 inline-flex items-center justify-center rounded-full border-2 border-black px-3 py-1 text-sm font-semibold">{discountAmount > 0 ? 'Change promo code' : 'Apply promo code'}</button>
              </div>
            )}
          </div>
